...
async function bootstrap() {
  // NestFactory creates the application instance from the root module
  ...
  // cookie-parser lets us read req.cookies in controllers
  app.use(cookieParser());

  // Global prefix: every route becomes /api/xxx automatically
  app.setGlobalPrefix('api');

  // ValidationPipe automatically validates incoming request bodies
  // using class-validator decorators defined in DTO files
  app.useGlobalPipes(
    new ValidationPipe({
  ...
  // Global exception filter: catches all thrown errors and formats them as
  // { success: false, message: "...", statusCode: NNN }
  app.useGlobalFilters(new HttpExceptionFilter());

  // Global interceptor: wraps every successful response as
